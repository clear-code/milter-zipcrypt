...
build()
{
	...
	*)
    ...
    distribution_depend_packages=${script_base_dir}/${PACKAGE}-depended-packages.${codename}

    run cp $source_dir/${PACKAGE}-${VERSION}.tar.gz \
	${CHROOT_BASE}/$target/tmp/
    run rm -rf ${CHROOT_BASE}/$target/tmp/${PACKAGE}-debian
    run cp -rp $source_dir/debian/${code_name} \
    ...
    [ -f $distribution_depend_packages ] && \
	run cat ${script_base_dir}/${PACKAGE}-depended-packages.${codename} >> \
	    ${CHROOT_BASE}/$target/tmp/depended-packages
